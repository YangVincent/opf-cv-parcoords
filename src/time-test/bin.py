...
def gaus_filter(bins, num_bins):
    ...
    pl.xlabel("$x$")
    pl.ylabel("$y$")
    pl.savefig("result/array.png")
    pl.imshow(res, cmap='Blues', interpolation='nearest')
    pl.xlabel("$x$")
    pl.ylabel("$y$")
    pl.savefig("result/array2.png")
...
def getBuckets(val1, val2, min_dim_i, max_dim_i, min_dim_j, max_dim_j, num_bins):
    bucket1 = int((float(val1) - float(min_dim_i)) / (float(max_dim_i) - float(min_dim_i)) * 10)
    bucket2 = int((float(val2) - float(min_dim_j)) / (float(max_dim_j) - float(min_dim_j)) * 10)
    if bucket1 == num_bins:
        bucket1 -= 1
    if bucket2 == num_bins:
        bucket2 -= 1
...
#filename = 'data/AirPassengers'
#filename = 'data/EuStockMarkets'
#filename = 'data/quakes'
#filename = 'data/credit-default'
filename = 'data/cleaned-midyear-population-age-country-code'
...
if __name__ == '__main__':
    ...
    fold = 'result/'
    output(fold + 'normalized_trends.txt', axesbinsnormalized)
    output(fold + 'outliers.txt', axesoutliers)
    output(fold + 'clusters.txt', clusterbins)
    writejson(fold + 'output.json', axesbinsnormalized, axesoutliers, clusterbins)
